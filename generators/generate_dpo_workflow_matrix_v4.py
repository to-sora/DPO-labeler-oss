"""V4 DPO workflow generator (consolidated).

Emits 4 YAMLs: ``v4_compact_same/diff``, ``v4_cinematic_same/diff``.
"""
...
DEFAULT_OUTPUT_DIR = WORKFLOWS_DIR / "dpo_workflow_v4"

PROMPT_AXES_SHORT = {"hybrid_compact": "compact", "hybrid_cinematic": "cinematic"}

RESEARCH_TEMPLATE_IDENTIFIERS: Dict[str, Dict[str, str]] = {
    "hybrid_compact": {
        "illustration": "research_v4/hybrid_compact_illustration_adult",
        "sdxl_anime_base": "research_v4/hybrid_compact_sdxl_anime_base_adult",
        "pony": "research_v4/hybrid_compact_pony_adult",
        "realistic": "research_v4/hybrid_compact_realistic_adult",
    },
    "hybrid_cinematic": {
        "illustration": "research_v4/hybrid_cinematic_illustration_adult",
        "sdxl_anime_base": "research_v4/hybrid_cinematic_sdxl_anime_base_adult",
        "pony": "research_v4/hybrid_cinematic_pony_adult",
        "realistic": "research_v4/hybrid_cinematic_realistic_adult",
    },
}
...
def make_template_args(prompt_axis: str, *, template_root: str, wildcard_root: str) -> Dict[str, Any]:
    if prompt_axis not in RESEARCH_TEMPLATE_IDENTIFIERS:
        raise ValueError(f"Unsupported V4 prompt axis {prompt_axis!r}")
...
def generate_matrix(output_dir: str | Path = DEFAULT_OUTPUT_DIR) -> List[Dict[str, Any]]:
    ...
    manifest = emit_version_matrix(version_tag="v4", templates=templates, output_dir=out_dir)
    write_yaml(out_dir / "manifest.yaml", {"workflow_count": len(manifest), "workflows": manifest})
    return manifest


def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate the V4 DPO workflow YAMLs.")
