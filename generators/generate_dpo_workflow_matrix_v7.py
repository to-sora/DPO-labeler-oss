...
DEFAULT_WILDCARD_DIR = WILDCARD_DIR / "research_v7"
DEFAULT_TEMPLATE_DIR = PROMPT_TEMPLATES_DIR / "research_v7"
DEFAULT_WORKFLOW_DIR = WORKFLOWS_DIR / "dpo_workflow_v7"
DEFAULT_GLOBAL_SEED_V7 = 20260328
DEFAULT_SESSION_COUNT = SHARED_SESSION_COUNT
DEFAULT_STEPS = 28
TEMPLATE_OMITTED_SOURCE_FILES = ("Character.txt", "characters/*")
...
def _token_for(relative_path: str | Path) -> str:
    ...
    return f"__research_v7/{path.with_suffix('').as_posix()}__"
...
def build_template_lines(set_name: str, family: str, source_files: list[str], *, raw_dir: Path) -> list[str]:
    ...
    if family == "illustration":
        ...
        return [
            ", ".join(
                [prefix]
                + _ordered_tokens(raw_dir, set_name, left_names)
                + ["BREAK"]
    ...
    if family == "sdxl_anime_base":
        ...
        return [
            ", ".join(
                _ordered_tokens(raw_dir, set_name, body_names)
    ...
    if family == "pony":
        ...
        body_names = common_order + extras
        return [
            ", ".join([prefix] + _ordered_tokens(raw_dir, set_name, body_names))
            for prefix in prefixes
        ]
...
def build_templates(template_output_dir: Path, *, raw_dir: Path, source_files: list[str], empty_files: list[str]) -> None:
    ...
    readme_lines = [
        ...
        "Template assembly omits character sources and uses top-level fallback only for:",
        "- semantic_class.txt",
        "",
...
def make_prompt_args(set_name: str, *, template_root: str, wildcard_root: str) -> Dict[str, Any]:
    return {
        ...
        "template": f"research_v7/{set_name}_hybrid_compact_illustration",
        "template_by_ckpt_family": {
            "illustration": f"research_v7/{set_name}_hybrid_compact_illustration",
            "sdxl_anime_base": f"research_v7/{set_name}_hybrid_compact_sdxl_anime_base",
            "pony": f"research_v7/{set_name}_hybrid_compact_pony",
            "realistic": f"research_v7/{set_name}_hybrid_compact_realistic",
        },
...
def generate_v7_trials(
    *,
    ...
    global_seed: int = DEFAULT_GLOBAL_SEED_V7,
    ...
    if not raw_dir.is_dir():
        raise ValueError(f"Missing required V7 source directory: {raw_dir}")
    ...
    workflow_rows = emit_version_matrix(
        version_tag="v7", templates=entries, output_dir=workflow_dir
    )
    ...
    _shared_write_yaml(
        workflow_dir / "manifest.yaml",
        {
            ...
            "top_level_template_fallbacks": ["semantic_class.txt"],
        },
    )
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate V7 trial assets from runtime-first V7 wildcard packs with top-level fallback."
    )
    ...
    parser.add_argument("--global-seed", type=int, default=DEFAULT_GLOBAL_SEED_V7)
...
def main() -> None:
    ...
    print(f"Using runtime-first V7 wildcards from: {result['raw_wildcard_dir']}")
    print(f"Wrote V7 templates to: {result['template_output_dir']}")
    print(f"Wrote V7 workflows to: {result['workflow_output_dir']}")
